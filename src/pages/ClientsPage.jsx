...
export default function ClientsPage() {
  const { clients, projects, payments } = useWorkspace();
      ...
      {visible.length ? (
        <div className="data-table">
          ...
          {visible.map((x) => {
            ...
            return (
              <Link className="data-table__row data-table__row--link" to={`/app/projects?client=${x.id}`} key={x.id}>
                <b>{x.name}</b>
                <span>{x.contact}<br /><small>{x.email}</small></span>
                <span>{stats.active} live / {stats.completed} done</span>
                <span>{formatMoney(stats.outstanding)}</span>
